...
def build_and_upload_dashboard():
    ...
    jobs_cursor.execute("""
        SELECT id, title, company, location, url, match_score, 
               date_posted, job_type, description, requirements, salary_range
        FROM jobs 
...
<head>
    ...
    <title>Job Search Dashboard - Updated {datetime.now().strftime("%Y-%m-%d %H:%M")}</title>
    <style>
        body {{
...
'''
    ...
    for job in jobs:
        ...
        <div class="job-card">
            ...
            <button class="btn btn-secondary" id="btn-{job_id}" onclick="toggleDetails({job_id})">Show Details ▼</button>
    ...
    try:
        ...
        if creds:
            ...
            results = service.files().list(
                q="name='dashboard.html' and trashed=false",
                fields="files(id, name)"
            ).execute()

            files = results.get('files', [])

            media = MediaFileUpload('dashboard.html', mimetype='text/html')
            ...
            if files:
                ...
                print(f"✅ Updated dashboard in Google Drive")
            else:
                # Create new file
                file_metadata = {'name': 'dashboard.html'}
                service.files().create(
                    body=file_metadata,
                    media_body=media,
                    fields='id'
                ).execute()
                print(f"✅ Uploaded new dashboard to Google Drive")
    except Exception as e:
        print(f"⚠️ Could not upload to Drive: {e}")
        print(f"📍 Dashboard saved locally: dashboard.html")

if __name__ == "__main__":
    build_and_upload_dashboard()
